...
let Brashkie = `${plugins.botAdmin || plugins.admin || plugins.group || plugins || noPrefix || hl ||  m.text.slice(0, 1) == hl || plugins.command}`
if (adminMode && !isOwner && !isROwner && m.isGroup && !isAdmin && Brashkie) return   
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '━═▣ 𝗛𝗲𝗽𝗲𝗶𝗻𝗕𝗼𝘁-𝗣𝗥𝗢 😻') :
...
"thumbnail": apii.data, 
"title": [wm, '𒁈 ℙℝ𝕆 ' + gt + ' 𒁈', '⚑ electronicatodo2006@gmail.com'].getRandom(),
"containsAutoReply": true,
"mediaType": 1, 
sourceUrl: 'https://github.com/Brashkie/BrashkieBot-Hepein' }}}, { quoted: fkontak2 })
apii.data = ''
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '𒁈 ℙℝ𝕆 ' + gt + ' 𒁈', ' 𒅒 electronicatodo2006@gmail.com'].getRandom(), thumbnail: BrasImg, sourceUrl: accountshb }}}}, tg)
