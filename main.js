...
global.authFile = 'HepeinBotSession';
global.authFileJB  = 'BrashkieJadiBot';
...
┊ ${chalk.blueBright('╰───────────➤𝗛𝗲𝗽𝗲𝗶𝗻𝗕𝗼𝘁-𝗣𝗥𝗢')}   
┊ ${chalk.blueBright('╭┈──────────────')}     
...
const connectionOptions = {
    ...
    mobile: MethodMobile, 
    browser: opcion == '1' ? ['HepeinBot-PRO', 'Edge', '20.0.04'] : methodCodeQR ? ['HepeinBot-PRO', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
    auth: {
        creds: state.creds,
...
if (!opts['test']) {
    if (global.db) setInterval(async () => {
        ...
        if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "BrashkieJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))
    }, 30 * 1000)
}
        if (opts['server']) (await import('./server.js')).default(global.conn, PORT)

        //respaldo de la sesión "HepeinBotSession"
            ...
            if (readRutaJadiBot.length > 0) {
                ...
                for (const gjbts of readRutaJadiBot) {
                    ...
                    if (readBotPath.includes(creds)) {
                        BrashkieJadiBot({pathBrashkieJadiBot: botPath, m: null, conn, args: '', usedPrefix: '/', command: 'serbot'})
                    }
                }
            }
        }
        ...
        global.reload = async (_ev, filename) => {
            if (pluginFilter(filename)) {
                ...
                if (filename in global.plugins) {
                    if (existsSync(dir)) conn.logger.info(`𒁈 SE ACTULIZADO - '${filename}' CON ÉXITO`)
                    else {
                        conn.logger.warn(`𒁈 SE ELIMINO UN ARCHIVO : '${filename}'`)
                        return delete global.plugins[filename];
                    }
                } else conn.logger.info(`𒁈 SE DETECTO UN NUEVO PLUGINS : '${filename}'`)
                const err = syntaxerror(readFileSync(dir), filename, {
                    sourceType: 'module',
                    allowAwaitOutsideFunction: true,
                });
                if (err) conn.logger.error(`𒁈 SE DETECTO UN ERROR DE SINTAXIS | SYNTAX ERROR WHILE LOADING '${filename}'\n${format(err)}`);
                else {
                    try {
                        const module = (await import(`${global.__filename(dir)}?update=${Date.now()}`));
                        global.plugins[filename] = module.default || module;
                    } catch (e) {
                        conn.logger.error(`𒁈 HAY UN ERROR REQUIERE EL PLUGINS '${filename}\n${format(e)}'`);
                    } finally {
        ...
        async function _quickTest() {
            const test = await Promise.all([
                ...
                return Promise.race([ new Promise((resolve) => { p.on('close', (code) => { resolve(code !== 127); }); }),
        ...
        async function purgeSession() {
            const sessionDir = './HepeinBotSession';
            try {
        ...
        async function purgeSessionSB() {
            const jadibtsDir = './BrashkieJadiBot/';
            try {
        ...
        async function purgeOldFiles() {
            const directories = ['./HepeinBotSession/', './BrashkieJadiBot/'];
            for (const dir of directories) {
                try {
